...
function generate(input: Input): Step<ListFrame>[] {
  ...
  frame(0, 0, { [nodes[0].id]: "active" }, `Floyd's tortoise & hare: slow and fast both start at the head (${nodes[0].value}).`, 1);
  ...
  while (fast !== null && nextIdx[fast] !== null && guard < 4 * n + 10) {
    ...
    fastMoves += 2;
    if (fast === null) {
      frame(slow, null, { [nodes[slow!].id]: "active" }, `Fast walked off the end → the list is acyclic. No cycle exists.`, 3);
      break;
    }
    if (slow === fast) {
      met = true;
      frame(slow, fast, { [nodes[slow!].id]: "found" }, `slow and fast met at ${nodes[slow!].value}! A cycle is present.`, 4);
      break;
    }
    frame(
      slow,
  ...
  if (met) {
    ...
    let p1: number = 0;
    let p2: number = slow!;
    frame(p1, p2, { [nodes[p1].id]: "active", [nodes[p2].id]: "active" }, `Phase 2: reset one pointer to the head. Advance both by 1 until they meet.`, 6);
    let g2 = 0;
    while (p1 !== p2 && g2 < n + 2) {
      g2++;
      p1 = nextIdx[p1]!;
      ...
      fastMoves++;
      if (p1 === p2) break;
      frame(p1, p2, { [nodes[p1].id]: "compare", [nodes[p2].id]: "compare" }, `Advance both: ${nodes[p1].value} vs ${nodes[p2].value}. Not the entrance yet.`, 7);
    }
    const done: Record<string, CellState> = {};
    // highlight the cycle
    for (let i = cyclePos; i < n; i++) done[nodes[i].id] = "visited";
    done[nodes[p1].id] = "found";
    frame(p1, p2, done, `Cycle entrance found at node ${nodes[p1].value} (index ${p1}). The highlighted nodes form the loop.`, 8);
  } else if (guard >= 4 * n + 10) {
    frame(slow, fast, {}, `Guard reached — stopping.`, 3);
  }
